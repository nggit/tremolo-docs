...
{: .warning }
**A caveat about sync handlers:** [Handler cancellation](#handler-cancellation) like the previous topic will not work as expected; it does not propagate to the thread. Even if the client is disconnected, tasks on the thread cannot be stopped as **there is no way to kill the thread from the outside**. You may encounter thread pool starvation, a blocked situation **if you do not make sure your code terminates**. Although this case does not degrade the async part of the server.
